...
from BNext.src.bnext import FFN_3x3, HardBinaryConv, HardSign
...
class ADM_ExtraBlock(nn.Module):
    ...
    def __init__(
        ...
        self.relu = nn.PReLU()
        # Conv + BatchNorm + RelU
        self.conv2 =  FFN_3x3(planes, planes)
    ...
    def forward(self, x):
        ...
        out = self.conv1(self.binary_activation(x))
        out = self.bn1(out)
        out = self.relu(out)
...
class ADM_EndBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1):
        ...
        # stride/2 maybe applied on conv1
        self.binary_activation_1 = HardSign(range = [-1.5, 1.5])
        self.conv1 = HardBinaryConv(
            inplanes, planes, kernel_size=kernel_size, stride=stride)

        self.relu = nn.PReLU()
        # Conv + BatchNorm + RelU
        self.binary_activation_2 = HardSign(range = [-1.5, 1.5])
        self.conv2 = HardBinaryConv(planes, planes, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        out = self.conv1(self.binary_activation_1(x))
        out = self.relu(out)

        out = self.conv2(self.binary_activation_2(out))
